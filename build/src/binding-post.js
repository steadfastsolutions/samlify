    ...
    if (metadata && metadata.idp && metadata.sp) {
        ...
        if (spSetting.loginRequestTemplate) {
            ...
            rawSamlRequest = libsaml_1.default.replaceTagsByValue(libsaml_1.default.defaultLoginRequestTemplate.context, {
                ...
                NameIDFormat: urn_1.namespace.format[spSetting.logoutNameIDFormat] || urn_1.namespace.format.emailAddress,
            });
        }
        if (metadata.idp.isWantAuthnRequestsSigned()) {
            var privateKey = spSetting.privateKey, privateKeyPass = spSetting.privateKeyPass, signatureAlgorithm = spSetting.requestSignatureAlgorithm;
            return {
                id: id,
            ...
            switch (_a.label) {
                case 0:
                    ...
                    tvalue = {
                        ...
                        NameID: user.email || '',
                        InResponseTo: lodash_1.get(requestInfo, 'extract.authnrequest.id') || '',
                        AuthnStatement: '',
                        AttributeStatement: '',
                    };
                    ...
                    if (!encryptThenSign && (spSetting.wantMessageSigned || !metadata.sp.isWantAssertionsSigned())) {
                        rawSamlResponse = libsaml_1.default.constructSAMLSignature(__assign({}, config, { rawSamlMessage: rawSamlResponse, isMessageSigned: true, signatureConfig: spSetting.signatureConfig || {
                                prefix: 'ds',
                ...
                case 3: throw new Error('Missing declaration of metadata');
            }
        });
    });
}
    ...
    if (metadata && metadata.init && metadata.target) {
        ...
        if (entity.target.entitySetting.wantLogoutResponseSigned) {
            ...
            return {
                id: id,
                context: libsaml_1.default.constructSAMLSignature({
                    referenceTagXPath: libsaml_1.default.createXPath('Issuer'),
                    privateKey: privateKey,
                    privateKeyPass: privateKeyPass,
                    signatureAlgorithm: signatureAlgorithm,
