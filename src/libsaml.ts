...
const libSaml = () => {
   ...
   */
  function getInnerTextWithOuterKey(
    xmlDoc,
    localName: string,
    localNameKey: string,
    valueTag: string
    ...
    selection.forEach(_s => {
      ...
      const key = select(
        "//*[local-name(.)='" + localName + "']/@" + localNameKey,
        xd
      );
  ...
  function getAttributeKey(
    ...
    selection.forEach(_s => {
      ...
      const value = select(
        "//*[local-name(.)='" + localName + "']/@" + attributeTag,
        xd
      );
  ...
  return {
    ...
    attributeStatementBuilder(attributes: LoginResponseAttribute[]): string {
      const attr = attributes
        .map(({ name, nameFormat, valueTag, valueXsiType }) => {
          return `<saml:Attribute Name="${name}" NameFormat="${nameFormat}"><saml:AttributeValue xsi:type="${valueXsiType}">{${tagging(
            "attr",
            valueTag
    ...
    verifySignature(xml: string, opts: SignatureVerifierOptions) {
      ...
      selection.forEach(s => {
        ...
        if (opts.keyFile) {
          ...
          if (selectedCert === "") {
            throw new Error(
              "certificate in document is not matched those specified in metadata"
            );
          }
    ...
    verifyMessageSignature(
      ...
      const key = new nrsa(utility.getPublicKeyPemFromCertificate(signCert), {
        signingScheme
      });
    ...
    getKeyInfo(x509Certificate: string, signatureConfig: any = {}) {
      this.getKeyInfo = key => {
        const prefix = signatureConfig.prefix
    ...
    encryptAssertion(sourceEntity, targetEntity, xml: string) {
      ...
      return new Promise<string>((resolve, reject) => {
        if (xml) {
          ...
          const assertionNode = getEntireBody(
            new dom().parseFromString(xml),
            "Assertion"
          );
          ...
          if (assertions.length !== 1) {
            throw new Error(
              `undefined number (${assertions.length}) of assertion section`
            );
          }
          ...
          if (sourceEntitySetting.isAssertionEncrypted) {
            xmlenc.encrypt(
              assertions[0].toString(),
              {
                ...
                encryptionAlgorithm:
                  sourceEntitySetting.dataEncryptionAlgorithm,
                keyEncryptionAlgorighm:
                  sourceEntitySetting.keyEncryptionAlgorithm
              },
              (err, res) => {
                if (err) {
                  return reject(
                    new Error("exception in encrpytedAssertion " + err)
                  );
                }
                if (!res) {
                ...
                const {
                  encryptedAssertion: encAssertionPrefix
          ...
          return reject(
            new Error("empty or undefined xml string during encryption")
          );
        }
      });
    },
    ...
    decryptAssertion(here, entireXML: string) {
      return new Promise<string>((resolve, reject) => {
        ...
        if (!entireXML) {
          return reject(
            new Error("empty or undefined xml string during decryption")
          );
        }
        ...
        if (encryptedAssertions.length !== 1) {
          throw new Error(
            `undefined number (${encryptedAssertions.length}) of encrypted assertions section`
          );
        }
